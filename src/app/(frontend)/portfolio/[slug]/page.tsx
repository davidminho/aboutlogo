...
export default async function CMSProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const project = portfolioProject || await getPublicProjectBySlug(slug)
  if (!project) notFound()

  return (
    <>
      <article className={`case-study shell cms-case ${portfolioProject ? `portfolio-case ${portfolioProject.orientation === 'landscape' ? 'landscape-case' : 'portrait-case'}` : ''}`}>
